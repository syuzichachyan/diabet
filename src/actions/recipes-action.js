import {RECIPES_FETCHING, RECIPES_FETCHING_FAILURE, RECIPES_FETCHING_SUCCESS} from '../constants/actions';
...
export const getRecipes = (excludes, includes) => (dispatch) => {
    ...
    includes.forEach(inclFoods => {
        excludes.forEach(food => exludecFoods = exludecFoods + `&excluded=${food}`);
        fetch(`https://api.edamam.com/search?q=${inclFoods}&app_id=28fb7256&app_key=b3bccf42eb282f3b21740bf3fa472af3&from=0&to=3&in` + exludecFoods).then(
            recipes => recipes.json()).then(recipes => {
